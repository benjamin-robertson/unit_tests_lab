...
require 'spec_helper'
require 'facter'
require 'facter/deployment_tier'

describe :deployment_tier, type: :fact do
  subject(:fact) { Facter.fact(:deployment_tier) }
  ...
  before :each do
    ...
    # allow(Facter).to receive(:value).with(:fqdn).and_return('test.example.com')
    # allow(Facter.fact(:ec2_metadata)).to receive(:value).and_return({'42'})
    # allow(Facter.fact(:node)).to receive(:value).and_return('blah-pupp-prd01')
  end

  it 'returns a value' do
    allow(Facter.fact(:hostname)).to receive(:value).and_return('blah-pupp-dev01')
    allow(Facter).to receive(:value).with(:hostname).and_return('blah-pupp-dev01')
    expect(fact.value).to eq('development')
  end


  it 'returns a value' do
    allow(Facter.fact(:hostname)).to receive(:value).and_return('blah-pupp-prd01')
    allow(Facter).to receive(:value).with(:hostname).and_return('blah-pupp-prd01')
    expect(fact.value).to eq('production')
  end
