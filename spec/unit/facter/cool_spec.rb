...
require 'facter'
require 'facter/cool'
# require 'facter/cloud_properties'

describe :cool, type: :fact do
  subject(:fact) { Facter.fact(:cool) }
  ...
  it 'returns a value' do
    allow(Facter.fact(:cloud_properties)).to receive(:value).and_return({'host' => { 'grouping' => 'blah_v1' }})
    expect(fact.value).to eq('blah_v1')
  end
end
